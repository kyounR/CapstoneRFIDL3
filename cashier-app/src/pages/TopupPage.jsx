...
function TopupPage() {
  ...
  function clearImmediateTopupResult() {
    setTopupResult(null)
    setReversalReason('')
    setReversalError('')
  }
      ...
      <form onSubmit={handleSubmit} className="card">
        ...
        <div style={{ marginBottom: '12px' }}>
          ...
          <input
            ...
            type="text"
            value={cardUid}
            onChange={(event) => { clearImmediateTopupResult(); setCardUid(event.target.value) }}
            className="input"
            style={{ width: '100%', marginTop: '4px' }}
            required
          />
        ...
        <div style={{ marginBottom: '12px' }}>
          ...
          <input
            ...
            min="0.01"
            value={amount}
            onChange={(event) => { clearImmediateTopupResult(); setAmount(event.target.value) }}
          ...
          {topupResult.id && !topupResult.is_reversed ? (
            <div style={{ marginTop: '12px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', marginTop: '8px' }}>
                ...
                {!reversalReason.trim() ? <span style={{ color: 'var(--text-secondary)', fontSize: '0.8rem' }}>Enter a reason to enable this button</span> : null}
              </div>
            </div>
          ) : null}
          {reversalError ? <p style={{ color: 'var(--danger)' }}>{reversalError}</p> : null}
